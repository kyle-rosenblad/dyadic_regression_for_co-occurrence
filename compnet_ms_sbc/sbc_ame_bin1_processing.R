...
library(rstan)
md <- readRDS("sbc_ame_bin1_out.RDS")
intercept <- md[[1]][[4]][[1]]
sigma <- md[[1]][[4]][[2]]
beta_trait1_sp <- md[[1]][[4]][[3]]
beta_trait1_dist <- md[[1]][[4]][[4]]
beta_trait2_sp <- md[[1]][[4]][[5]]
beta_trait2_dist <- md[[1]][[4]][[6]]
for(v in 1:16){
  ...
  for(i in 1:length(md)){
    ...
    srmpars_tmp <- data.frame(rep=i,
                              ...
                              sigma=tmpdraws$sigma_species_randint,
...
library(patchwork)
intercept_plot+sigma_plot+beta_trait1_sp_plot+beta_trait1_dist_plot+
  beta_trait2_sp_plot+beta_trait2_dist_plot+latfacterm_plot
ggsave("../output_files/sbc_ame_bin1_plot.png", height=10, width=10)
