...
# For simplicity, use 100 trials for each dyad
d$either <- 100
...
alpha <- -0.3
sigma <- 1.1
trait1_trait2_corr <- 0.2
beta_trait1_sp <- 1
beta_trait1_dist <- -1.9
beta_trait2_sp <- 0.1
beta_trait2_dist <- 0.9
...
# label-switching.
multi_effects_corr <- 0.1
multi_effects_sigma <- c(0.4, 0.2)
...
d$pboth <- expit(alpha + d$avalA + d$avalB +
...
d$both <- rbinom(n=nrow(d), size=d$either, prob=d$pboth)
...
stanmod_dummy <- stan(file="ame_binomial.stan",
                      data=datalist,
                      cores=1,
                      chains=1,
                      warmup=1,
...
sbc_mods_and_data <- foreach(u=1:16) %dopar% {
  ...
  stanmod <- stan(file="ame_binomial.stan",
                  fit=stanmod_dummy,
  ...
  truepars <- list(alpha, sigma, beta_trait1_sp, beta_trait1_dist, beta_trait2_sp, beta_trait2_dist, latfacs)
  tmpout <- list(stanmod, d, datalist, truepars)
  tmpout
}
saveRDS(sbc_mods_and_data, "sbc_ame_bin2_out.RDS")
