...
res <- foreach(u=1:100) %dopar% {
  ...
  sp_df$domtrait <- c(scale(rnorm(n=nrow(sp_df))))
  sp_df$zi <- c(scale(rnorm(n=nrow(sp_df))))
  sp_df$ai <- 0.01
  sp_df$sigmai <- 1
  
  # Generate a matrix of alpha values defining the strength of competition within and between species
  # Closer phylogenetic relatives compete more strongly
  library(geiger)
  library(phytools)
  tree <- drop.extinct(sim.bdtree(b=2, d=1, stop=c("taxa"), n=species, extinct=FALSE, seed=i))
  tree$tip.label <- sp_df$sp
  comp_phy <- as.matrix(cophenetic(tree)) * 0.5 # The arbitrary default phylogenetic distance units are a bit too big, yielding huge distances and thus tiny competition effects. Multiplying by 0.5 here makes the scale more reasonable.
  domtrait_dist <- t(outer(sp_df$domtrait, sp_df$domtrait, `-`))
  comp_dist <- comp_phy + domtrait_dist
  alpha_mat <- 0.05*(u/100)*exp(-comp_dist)
