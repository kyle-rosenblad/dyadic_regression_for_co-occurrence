...
mod2 <- buildcompnet(presabs = mat,
                     ...
                     adapt_delta=0.95,
                      olre=FALSE)
ppred2 <- postpredsamp(mod2) # breaks BiasedUrn indicating bad fit

output[1, "model"] <- "fnch"
output[1, "diagnostics"] <- "poor"
output[1, "runtime"] <- sum(rstan::get_elapsed_time(mod2$stanmod))
...
output[2, "runtime"] <- sum(rstan::get_elapsed_time(mod2$stanmod))
...
output[3, "diagnostics"] <- "good"
output[3, "runtime"] <- sum(rstan::get_elapsed_time(mod1$stanmod))


saveRDS(output, "modtest_summary_corr1.RDS") # # #
