...
mod2 <- buildcompnet(presabs = mat,
                     ...
                     adapt_delta=0.99,
                      olre=FALSE)
ppred2 <- postpredsamp(mod2) # broke BiasedUrn; degenerate
...
mod1 <- buildcompnet(presabs = mat,
                     ...
                     family='fnchypg',
                     adapt_delta=0.9)
ppred1 <- postpredsamp(mod1) # Broke BiasedUrn; degenerate

output[2, "model"] <- "fnch_olre"
output[2, "diagnostics"] <- "poor"
output[2, "runtime"] <- sum(rstan::get_elapsed_time(mod0$stanmod))
...
saveRDS(output, "modtest_summary_corr11.RDS") # # #
